...
var writeData = function(client, type, message_parts) {
  ...
  switch(type) {
  case "clickedObj" :
    console.log("writing data, the case is clickedObj")
  ...
  case "message" :
    ...
    console.log("writingData, the case is message")
    newVar = 'other content'
    console.log(msg.split('&'))
    msgSplit=msg.split('&')
    typedLabel = msgSplit[0]
    if(msgSplit.length>1){
      targetObject = msgSplit[1] 
    } else {
      targetObject = null
    }
     if(msgSplit.length>2) {
      speakerClicked = msgSplit[2]
    } else {
      speakerClicked = null
    } 

    var line = (id + ',' + Date.now() + ',' + roundNum + ',' + client.role + ',' + msg + ',' + targetObject + ',' + speakerClicked+ '\n');
...
var startGame = function(game, player) {
  ...
  utils.establishStream(game, "message", dataFileName,
			"gameid,time,roundNum,sender,contents,myVar\n");
