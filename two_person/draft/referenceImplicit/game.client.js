...
var client_onserverupdate_received = function(data){  
  ...
  if(data.players.length > 1) {
    console.log("multiple players, time to get going!")
    $('#messages').empty();    
    $("#chatbox").removeAttr("disabled");
    $('#chatbox').focus();
    globalGame.get_player(globalGame.my_id).message = "";
    $('#onlyOnePlayer').hide()
    //let the speaker send a message/play now
    
    //debugging gameplay with these two lines
...
var client_onjoingame = function(num_players, role) {
  ...
  $("#gameTargetShared").val(imgArray[0])
