...
function getRandomImages(imgAr, path, gameOrAttention, count) {
    ...
    if (gameOrAttention=="game") {
    	classes = " col-xs-3 col-lg-2 toSelect toSelect2";
...
setTimeout(function() {console.log(game_player)}, 2000)
...
var experiment = {
	...
	game: function(score, roundNumber, slideNumber) {
		  // set role locally
		  setTimeout(function() {text_content = document.getElementById('roleLabel').lastChild
		...
		setTimeout(function() {
			if(role == "speaker.") {
				...
				console.log("speaker, target = "+ $("#gameTargetShared").val())
			      $("#chatbox").hide()
				...
				document.getElementById("gameTarget").innerHTML = '<img class="col-xs-8 col-xs-offset-3" style="max-width:500px" id="orderedImage" src="' + basePath + $("#gameTargetShared").val() + '" alt = "'+$("#gameTargetShared").val()+'">'
				var target = pairObjectLabels($("#gameTargetShared").val());
			...
			///// code for the receiver/listener version of the game
				...
				console.log("listener, target = "+ $("#gameTargetShared").val())
			    $("#referenceGameReceiver").fadeTo("slow", 0.5)
				time1 = new Date().getTime();
				$("#chatarea").hide();

				showSlide("referenceGameReceiver");
				...
				document.getElementById("sendMessage2").innerHTML = "Next Round";

				$("#spinningWaiting2").show();
				...
				var message = 0; // no message
				document.getElementById("objects22").innerHTML = getRandomImages(imgArray, basePath, "game", roundNumber);
				$('.toSelect').click(function() {
						//if neither pointing nor typing has occured, select the target element and note that.
						if (message==0) {
							this.style.border="5px solid black";
